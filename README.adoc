...
Note that there is no `clock` signal in RHDL, because the `Synchronous` trait handles that for us. 


 I will try to implement a simple UART receiver that outputs a byte when a start bit is detected. I will use the following parameters for the UART:
== TODO
