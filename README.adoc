...
[source,rust]
----
include::src/rhdl_blinker_test.rs[blinker]
----
...
We want ot output verilog and also want to simulate/test our design directly with `cargo test`. As looking at test worked quite well previously I searched for a test that simulates and runs a design.
